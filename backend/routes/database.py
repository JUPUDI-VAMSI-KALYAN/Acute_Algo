...
async def test_database_connection():
    ...
    try:
        ...
        if test_repo:
            ...
            retrieved_repo = await db_service.get_repository_by_url(
            ...
            return {"success": False, "message": "Failed to create test repository"}
...
async def save_analysis_to_database(request: AnalysisRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to save analysis to database: {str(e)}"
        )
...
async def get_repositories(limit: int = 10):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get repositories: {str(e)}"
        )
...
async def get_repositories_summary(limit: int = 10):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get repositories summary: {str(e)}"
        )
...
async def get_repository_functions(
    ...
    try:
        ...
        logger.info(
            f"Found {functions_data.get('total', 0)} total functions, returning page {page}"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get repository functions: {str(e)}"
        )
...
async def get_repository_files(repository_id: int, page: int = 1, limit: int = 20):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get repository files: {str(e)}"
        )
...
async def save_ai_analysis(request: dict):
    ...
    try:
        ...
        result = await db_service.create_ai_analysis(
            function_id=int(function_id), enhanced_data=enhanced_data
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to save AI analysis: {str(e)}"
        )
...
async def get_function_ai_analysis(function_id: int):
    ...
    try:
        ...
        return {"success": True, "data": function_data}
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error getting function AI analysis: {e}")
        raise HTTPException(
            status_code=500, detail=f"Failed to get function AI analysis: {str(e)}"
        )
...
async def get_algorithm_ai_analysis(algorithm_id: int):
    ...
    try:
        ...
        if not algorithm_data.get("is_algorithm", False):
            raise HTTPException(
                status_code=404, detail="Function is not classified as an algorithm"
            )

        return {"success": True, "data": algorithm_data}
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error getting algorithm AI analysis: {e}")
        raise HTTPException(
            status_code=500, detail=f"Failed to get algorithm AI analysis: {str(e)}"
        )
...
async def create_chat_conversation(request: dict):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to create chat conversation: {str(e)}"
        )
