...
async def analyze_repository(request: AnalysisRequest):
    ...
    try:
        ...
        if scan_result.function_analysis:
            ...
            avg_algorithms_per_file = round(fa.total_algorithms / fa.total_files, 2) if fa.total_files > 0 else 0
...
async def analyze_and_save_repository(request: AnalysisRequest):
    ...
    try:
        logger.info(f"Starting comprehensive analysis for repository: {request.github_url}")
        ...
        try:
            ...
            logger.info(f"Comprehensive analysis completed and saved for repository: {repo_id}")
