...
async def analyze_function(request: AIAnalysisRequest):
    ...
    try:
        ...
        logger.info(f"Starting AI analysis for function '{request.function_name}' with type: {analysis_type}")
        ...
        logger.info(f"Function classification: is_algorithm={is_algorithm}, score={score}")
        ...
        if analysis_type == "comprehensive":
            ...
            if isinstance(analysis_result, ComprehensiveAnalysisResult):
                ...
                response_data = AIAnalysisData(
                    ...
                    optimizationSuggestions=tech_analysis.optimization_suggestions or [],
                    ...
                    maintenanceComplexity=business_analysis.maintenance_complexity or '',
            ...
            if isinstance(analysis_result, ComprehensiveAnalysisResult):
                ...
                response_data = AIAnalysisData(
                    ...
                    complexityAnalysis=getattr(analysis_result, 'complexity_analysis', '') or '',
                    optimizationSuggestions=getattr(analysis_result, 'optimization_suggestions', []) or [],
                    potentialIssues=getattr(analysis_result, 'potential_issues', []) or [],
                    analysisType=analysis_type,
                    shortDescription=getattr(analysis_result, 'short_description', '') or '',
                    analysisTimestamp=datetime.now().isoformat()
                )
        ...
        if request.function_id:
            try:
                ...
                stored_analysis = await db_service.create_ai_analysis(
                    ...
                    enhanced_data=analysis_dict
                )
                
                if stored_analysis:
                    logger.info(f"AI analysis stored in database for function ID {request.function_id}")
                else:
                    logger.warning(f"Failed to store AI analysis in database for function ID {request.function_id}")
                    
            except Exception as db_error:
                logger.error(f"Database storage error for function ID {request.function_id}: {db_error}")
                # Don't fail the entire request if database storage fails
        
        logger.info(f"AI analysis completed successfully for function '{request.function_name}'")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to analyze function: {str(e)}"
        )
...
async def chat_with_ai(request: ChatRequest):
    ...
    try:
        ...
        if not response_text or response_text.startswith("AI service is not available") or response_text.startswith("Chat failed"):
            raise HTTPException(
                status_code=500,
                detail=response_text or "Failed to generate AI response. Please try again."
            )
